...
Click the toolbar icon to open the quick-settings popup, right-click the icon and select **Options**, or click the `[options]` link in the Boomi footer bar. Changes take effect after clicking **Save** and reloading the Boomi platform tab.
</details>
...
- **Variable declarations** — top-level declarations referenced by other files in the bundle must use `var` (e.g., `var BoomiPlatform`, `var renderBoomiModal`, shared SVG assets). File-local declarations should use `const` or `let`. Never use implicit globals (assignment without a declaration keyword).
- **CSS convention** — all styling goes in `library/css/boomi.css`. Never use inline `style=""` attributes, `element.style.*` in JS, or `<style>` blocks. Use classes (`element.classList.add`/`remove`) and define the rules in `boomi.css`. This applies to every file — options page, content scripts, everything. Computed/dynamic styles (e.g., drag positioning, transform scaling) are an acceptable exception when the value cannot be known at CSS authoring time.
