...
| `content/runtimePanel.js` | content | Runtime status panel — footer dots and slide-out panel on the build canvas showing Atom/Cloud/Molecule status, version, and type. Queries `Atom/query` via background service worker. Reads `runtime_status_panel` from BoomiPlatform config. |
