...
| `content/componentReferences.js` | content | Component references button (🔗) in the build canvas header. Opens a modal showing "Used By" (parents) and "References" (children) from the ComponentReference API with click-to-open links. Reads `component_references` from BoomiPlatform config. |
...
| `content/setPropertiesExtractor.js` | content | Build toolbar button that extracts Set Properties shapes (API-based with multi-tab Process Analysis: Notify, Messages, SQL, Decisions, Maps, Scripts, Inventory) via a single XML parse, falling back to DOM extraction when no API token is available |
